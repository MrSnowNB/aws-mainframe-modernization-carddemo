...
# -------------------------------------------------------------------
# COBOL fixed format: cols 1-6 = sequence, col 7 = indicator (* = comment),
# cols 8-11 = area A (level numbers live here), cols 12-72 = area B.
# A Level-01 declaration looks like:  "       01  NAME ..."
_L01_RE = re.compile(
...
def extract_l01_items(src_path: Path) -> list[dict]:
    """
    Parse a COBOL fixed-format source file and return all Level-01
    data item declarations found in the DATA DIVISION.

    Returns a list of {"name": str, "level": 1} dicts in source order,
    de-duplicated by name.  Items defined only in COPYbooks are not
    included (they are not physically present in the .cbl file).
    """
    ...
    for raw_line in src_path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if not in_data_div:
            continue
        # Skip comment lines: indicator column (index 6) is '*'
