...
def compare(program_id: str) -> bool:
    ...
    # ── Check 2: Extra paragraphs in MD must be CFG dead-code ─────────────────
    extra_paras = cl_paragraphs - gt_reachable
    gt_dead = set(gt["paragraphs_dead"])
    cl_dead = set(cl["dead_declared_in_md"])
    # Extra paragraph is acceptable only if GT says it is dead code
    unexplained = sorted(extra_paras - gt_dead)
    if unexplained:
        failures.append({
            "check": "hallucinated_paragraphs",
            "detail": "MD claims paragraphs not in source and not in dead_code_paragraphs",
            "items": unexplained
        })
    ...
    # t04_semantic_score MUST be null until a judge process writes the report.
    # A non-null value here means the translator wrote its own score — FAIL.
    ...
    status = "PASS ✅" if gate_pass else f"FAIL ❌  ({len(failures)} failures, {len(warnings)} warnings)"
    print(f"[GATE] {program_id}: {status}")
    for f in failures:
        items_str = str(f.get("items", f.get("value", "")))
        print(f"  ✗ {f['check']}: {items_str}")
    for w in warnings:
        print(f"  ⚠  {w['check']}: {w['items']}")
...
if __name__ == "__main__":
    ...
    print(f"  {passed}/{total} programs passed")
    print("─────────────────────────────────────────────────────")
